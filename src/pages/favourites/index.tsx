  ...
  return (
    ...
    <Layout >
      ...
      {session && posts?.length==0 &&
        <div>
          <div className='flex justify-center flex-col rounded gap-4 items-center shadow-xl h-[400px]'>
            <p className='text-xl font-basic'>В списке не обнаружено товаров.</p>
            
            <Link className='p-3 px-6 bg-red-600 rounded text-white' href="/catalog">Перейти в каталог</Link>
          </div>
        ...
        {posts?.map((item)=>(
          <div className='min-h-[270px] w-full grid p-5 shadow-xl rounded-xl  border-solid border-base border' key={item.postsID}>
            <div className='flex items-center gap-10 justify-between'>
              <Link href={"/product/"+item.postsID}><img src={item.posts.imgs[0]?.altTitle} className='w-[150px] '/></Link>
              <div className=' max-w-[500px]'>
              <Link href={"/product/"+item.postsID}><p className="text-2xl">{item.posts.title}</p></Link>
                <p className="text-lg">{item.posts.property}</p>
              </div>              
              <div className='justify-end flex flex-col items-end '>
                <p className="text-2xl">{item.posts.price} Р</p>
                <button onClick={async (e)=>{
                       e.preventDefault()
                       mutation.mutateAsync(item.postsID)
                       }}>
                    <Image src={Favouritess} className="active:border-red-600 border-solid active:p-1 mt-20" alt="избранное"/>
                </button>
              </div>
            </div>
          </div>
