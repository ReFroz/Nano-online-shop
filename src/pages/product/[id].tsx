...
import { DOMAttributes, FormEventHandler, PropsWithChildren, useEffect, useState } from "react";
...
export default function Page() {
  ...
  const {data:comments}= api.comment.getCom.useQuery(
    Number(router.query.id)
  )
  if (allPost?.length == 0) router.push("/404");

  const title=allPost?.map((item)=>{return item.title})
  const [selectImage, setSelectImage] = useState("");

  const [com,SetCom]=useState("")
  const mutation=api.comment.addCom.useMutation()
    ...
    <Head>
      ...
      <Layout className="font-basic">
        <div className="mx-auto max-w-[1180px]">
          ...
          <div className="mt-10 flex flex-col gap-[30px] p-5">
          <p className="text-3xl">Комментарии</p>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5 max-w-[300px]">
          <textarea 
          className="p-5 resize-none overflow-form"
          value={com}
          onChange={(e)=>
          ...
          {comments?.map((com)=>(<>
            <div key={com.id} className="rounded shadow-xl">
              <div className="py-3 px-5 flex flex-col gap-2">
                <p className="text-2xl">{com.user.name}{" "}{com.user.lastName}:</p>
                <p>Комментарий</p>
                <p className="text-xl">"{com.text}"</p>
              </div>
            </div>
